...
import {
  ...
  ExternalLink, Copy, CheckCircle2, Clock, FolderKanban,
} from "lucide-react";
...
const controlCls =
  "rounded-xl border border-white/10 bg-white/5 px-4 py-2.5 text-sm text-white focus:border-sky-500 focus:outline-none focus:ring-2 focus:ring-sky-500/20 transition";
...
export default function ProjectsPage() {
  ...
  const duplicate = async (p: ProjectData) => {
    try {
      ...
      toast.error("Couldn't duplicate that project");
    }
  };
      ...
      <div className="flex flex-col lg:flex-row gap-3 mb-5">
        <div className="relative flex-1">
          ...
          <input
            ...
            placeholder="Search title, tags or tech"
        ...
        >
          <Star size={15} className={featuredOnly ? "fill-amber-300" : ""} /> Featured
        </button>
      </div>
      ...
      {projects.length === 0 ? (
        <Card>
          <EmptyState
            ...
            description="Add your first project to showcase your work on your portfolio."
            action={!isViewer ? "Add your first project" : undefined}
          ...
          {projects.map((p) => (
            <Card key={p._id} className="overflow-hidden flex flex-col transition hover:shadow-card-hover">
                  ...
                  <button
                    ...
                    className={`absolute top-2 left-2 inline-flex items-center gap-1 px-2 py-1 rounded-lg text-xs font-semibold transition ${
                      p.featured
                        ...
                        : "bg-black/50 text-white/60 hover:text-amber-300"
                    }`}
                  >
                    <Star size={12} className={p.featured ? "fill-amber-950" : ""} /> Featured
                  </button>
                )}
                {isViewer && p.featured && (
                  <span className="absolute top-2 left-2 inline-flex items-center gap-1 px-2 py-1 rounded-lg bg-amber-400/90 text-amber-950 text-xs font-semibold">
                    <Star size={12} className="fill-amber-950" /> Featured
                  </span>
                )}
                <span
              ...
              <div className="p-4 flex flex-col flex-1">
                ...
                <p className="text-sm text-white/60 mt-1 line-clamp-2 flex-1">{p.description}</p>

                {p.tags?.length > 0 && (
                  <div className="flex flex-wrap gap-1 mt-3">
                    {p.tags.map((tg) => (
                      <span key={tg} className="text-xs px-2 py-0.5 rounded bg-white/5 text-white/60">
                        #{tg}
                      </span>
                    ))}
                  </div>
                ...
                {p.technologies?.length > 0 && (
                  <div className="flex flex-wrap gap-1 mt-2">
                    {p.technologies.map((tc) => (
                      <span key={tc} className="text-xs px-2 py-0.5 rounded bg-white/5 text-white/60">
                        {tc}
                      </span>
                    ))}
                  </div>
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4 mt-6">
        <div className="flex items-center gap-2 text-sm text-white/60">
          ...
          <select
            ...
            className="rounded-lg border border-white/10 bg-white/5 px-2 py-1 text-white focus:border-sky-500 focus:outline-none"
          >
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="inline-flex items-center gap-1 px-3 py-1.5 rounded-lg border border-white/10 bg-white/5 text-sm text-white hover:bg-white/10 disabled:opacity-40 disabled:cursor-not-allowed transition"
          >
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-1.5">
            ...
            <input
              ...
              placeholder="fullstack, dashboard, saas"
              className={inputCls}
            />
          </div>
          ...
          <label className="flex items-center gap-2.5 text-white cursor-pointer">
            ...
            />
            <Star size={15} className="text-amber-400" /> Mark as featured
          </label>
